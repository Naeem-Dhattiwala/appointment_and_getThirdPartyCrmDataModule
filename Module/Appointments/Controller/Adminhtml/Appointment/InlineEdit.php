        ...
        if ($this->getRequest()->getParam('isAjax')) {
            ...
            if (!count($postItems)) {
                ...
                foreach (array_keys($postItems) as $modelid) {
                    /** @var \Iram\Appointments\Model\Appointment $model */
                    $model = $this->_objectManager->create(\Iram\Appointments\Model\Appointment::class)->load($modelid);
                    try {
                        $model->setData(array_merge($model->getData(), $postItems[$modelid]));
                        $model->save();
                    } catch (\Exception $e) {
                        $messages[] = "[Appointment ID: {$modelid}]  {$e->getMessage()}";
                        $error = true;
                    }
                }
            }
